...
export enum Twilight { Official, Astronomical, Nautical, Civil };
...
export class MeeusEngine {
    ...
    public static toGmsTime(date : UtcDate) : GmsTime {
    ...
    public static toLmsTime(date : UtcDate, location: GeographicCoordinates | Angle | number) : LmsTime {

        var longitude: any = location;
        
        if (longitude instanceof Angle) 
            longitude = (<Angle>longitude).degrees;
        else if (longitude instanceof GeographicCoordinates)
            longitude = (<GeographicCoordinates>longitude).longitude.degrees;
    ...
    public static toGasTime(date : UtcDate) : GasTime {
        return new GasTime(MeeusEngine.toGasDegrees(date) / 15.0);				
    }
    
    public static toLasTime(date : UtcDate, location: GeographicCoordinates | Angle | number) : LasTime {
    ...
    public static toHjdDate(date : UtcDate, location: GeographicCoordinates, position : EquatorialCoordinates) : HjdDate {
...
//							MeeusEngine.convertToHeliocentric(location, date));
    ...
    public static getSunriseAt(location : GeographicCoordinates, date?: UtcDate | Date, twilight?: Twilight) : UtcDate {
        return MeeusEngine.calc_SunriseOrSunset(true, MeeusEngine.prepareJd(date || new Date()), location, twilight || Twilight.Official);
    }
    
    public static getSunsetAt(location : GeographicCoordinates, date?: UtcDate | Date, twilight?: Twilight) : UtcDate {
    ...
    public static sunPositionAt(date : UtcDate) : EquatorialCoordinates{
    ...
    public static earthPositionAt(date : UtcDate) : EquatorialCoordinates {
        var params = MeeusEngine.preparePositionalParameters(date.centuriesSinceJ2000);
        var stl = params.sun.trueLongitude;
        var emo = params.ecliptic.meanObliquity;
    ...
    public static moonPositionAt(date : UtcDate) : EquatorialCoordinates {
    ...
    public static getAngularSeparation(ob1 : EquatorialCoordinates, ob2 : EquatorialCoordinates) : Angle {
        ...
        var y = Math.sqrt(
                        ((Math.cos(o2dec) * Math.sin(o1ra - o2ra))) * ((Math.cos(o2dec) * Math.sin(o1ra - o2ra))) +
    ...
    public static toHorizontalCoordinates(ec : EquatorialCoordinates, date: UtcDate | Date, location: GeographicCoordinates) : HorizontalCoordinates {
        var utc : UtcDate;
        var mst : LmsTime;
        ...
        // choose hemisphere
        if (Math.sin(haRad) > 0)  h_az = 360 - h_az;
...
//				console.log('toHorizontalCoordinates: ' + h_alt);
    ...
    public static toEquatorialCoordinates(position : HorizontalCoordinates, date: UtcDate | Date, location: GeographicCoordinates) : EquatorialCoordinates {
            var utc : UtcDate;
...
//				console.log('toEquatorialCoordinates: ' + position.altitude.degrees);
        ...
        return MeeusEngine.toJ2000(
            new EquatorialCoordinates(rightAscenstion, Angle.fromRadians(declination)), 
            utc);
    }
    ...
    static normalize(x : number) : number { return x - (360.0 * Math.floor(x / 360.0)); }
    
    //#endregion Math helpers
                
    //#region Cartesian Coordinates helpers
    
    private static addCartesian(o1: any, o2: any) : Vector {
        return {
            x: o1.x + o2.x,
            y: o1.y + o2.y,
            z: o1.z + o2.z	
    ...
    public static equatorialToCartesian(position: EquatorialCoordinates) : Vector {
    ...
    private static geographicToCartesian(location : GeographicCoordinates, date : UtcDate) : Vector {
    ...
    private static preparePositionalParameters(jt : number) : any {
    ...
    public static calc_Earth_OrbitEccentricity(jt : number) : number {
        return 0.016708634 - jt * (0.000042037 + 0.0000001267 * jt);
    }
    
    public static earthCartesianPositionAt(date : UtcDate) : Vector {
        var p1 = MeeusEngine.earthPositionAt(date);
        return MeeusEngine.equatorialToCartesian(p1);				
    }
    
    public static earthCartesianVelocityAt(date : UtcDate) : Vector { 
        var p1 = MeeusEngine.earthPositionAt(date);
    ...
    private static convertToHeliocentric(location: GeographicCoordinates, date : UtcDate) : Vector {
    ...
    private static calc_Ecliptic_MeanObliquity(jt : number, jt2? : number, jt3? : number) : number {
        jt2 = jt2 || jt * jt;
        jt3 = jt3 || jt2 * jt;
        return (21.448 / 60 + 26) / 60 + 23 + (-46.815 * jt - 0.00059 * jt2 + 0.001813 * jt3) / 3600; // degrees
    }
    
    private static calc_Ecliptic_ObliquityCorrection(jt : number, eclipticMeanObliquity? : number) : number {
    ...
    private static calc_Sun_Earth_Distance(jt : number, earthEcc? : number, sunMeanAnomaly? : number, sunEqOfCenter? : number, sunTrueAnomaly? : number) : number {
    ...
    private static calc_Sun_MeanLongitude(jt : number) : number {
        return MeeusEngine.normalize(280.46646 + jt * (36000.76983 + jt * 0.0003032));
    }
    
    private static calc_Sun_MeanAnomaly(jt : number, jt2? : number, jt3? : number) : number {
        jt2 = jt2 || jt * jt;
        jt3 = jt3 || jt * jt2;
    ...
    private static calc_Sun_EqOfCenter(jt : number, sunMeanAnomaly? : number) : number {
    ...
    private static calc_Sun_TrueLong(jt : number, sunMeanLongitude? : number, sunEqOfCenter? : number) : number {
    ...
    private static calc_Sun_TrueAnomaly(jt : number, sunMeanAnomaly? : number, sunEqOfCenter? : number) : number {
        var sma = sunMeanAnomaly || MeeusEngine.calc_Sun_MeanAnomaly(jt);
        var sec = sunEqOfCenter || MeeusEngine.calc_Sun_EqOfCenter(jt);
        return sma + sec; // degrees
    }
    
    private static calcSunRadVector(jt : number, sunTrueAnomaly? : number, earthOrbitEcc? : number) : number {
    ...
    private static calc_Sun_LongitudeOfAscendingNode(jt : number, jt2? : number, jt3? : number) : number {
        jt2 = jt2 || jt * jt;
        jt3 = jt3 || jt2 * jt;
    ...
    private static calc_Moon_MeanAnomaly(jt : number, jt2? : number, jt3? : number) : number {
        jt2 = jt2 || jt * jt;
        jt3 = jt3 || jt2 * jt;				
        return MeeusEngine.normalize(134.96298 + 477198.867398 * jt + 0.0086972 * jt2 + jt3 / 56250); // degrees
    }
    
    private static calc_Moon_MeanElongation(jt : number, jt2? : number, jt3? : number) : number {
        jt2 = jt2 || jt * jt;
        jt3 = jt3 || jt2 * jt;
    ...
    private static calc_Moon_MeanLongitude(jt : number) : number {
        return MeeusEngine.normalize(218.316 + 481267.8813 * jt);
    }
    
    private static calc_Moon_LongitudeAscendingNode(jt : number, jt2 : number, jt3 : number) {
        return MeeusEngine.normalize(125.045 - 1934.14 * jt + 0.002071 * jt2 + jt3/450000);
    }
    
    private static calc_Moon_Earth_Distance(moonMeanAnomaly : number, moonMeanElongation : number) {
    ...
    private static calc_Moon_Sun_DistanceRatio(moonEarthDistance : number, sunEarthDistance : number) : number {
        return (moonEarthDistance / sunEarthDistance) / 379.168831168831;
    }
    
    private static calc_Moon_GeocentricLatitude(moonArgOfLatitude : number, moonMeanAnomaly : number, moonMeanElongation : number) : number {
    ...
    private static calc_Moon_GeocentricLongitude(moonMeanAnomaly : number, moonMeanElongation : number, moonMeanLongitude : number, moonArgumentOfLatitude : number, sunMeanAnomaly : number) : number {
    ...
    private static calc_Moon_HeliocentricLatitude(moonSunDistanceRatio : number, moonGeocentricLatitude : number) : number {					
        return moonSunDistanceRatio * moonGeocentricLatitude;
    }
    
    private static calc_Moon_HeliocentricLongitude(sunApparentLongitude : number, moonSunDistanceRatio : number, moonHeliocentricLatitude : number, moonGeocentricLongitude : number) : number {
    ...
    private static calc_Moon_ArgumentOfLatitude(jt : number, jt2? : number, jt3? : number) : number {
        jt2 = jt2 || jt * jt;
        jt3 = jt3 || jt2 * jt;
        return MeeusEngine.normalize(93.27191 + 483202.017538 * jt - 0.0036825 * jt2 + jt3 / 327270);
    }			
    
    public static calc_Moon_LongitudeOfTerminator(u : UtcDate) : Angle {
    ...
    public static calc_Moon_IlluminatedDiscFraction(u : UtcDate) : number {
        var jt = u.centuriesSinceJ2000;
        var jt2 = jt * jt;
        var jt3 = jt * jt2;
    ...
    private static calc_Ecliptic_NutationInLongitude(jt : number, sunLongitudeOfAscendinNode : number, moonMeanElongation : number, moonArgumentOfLatitude : number, sunMeanAnomaly : number, moonMeanAnomaly : number) : number {
    ...
    private static calc_Ecliptic_MeanObliquityCorrection(jt: number, jt2 : number, jt3 : number, OMs : number, Ds : number, DFs : number, Ms : number, M1s : number) : number {
    ...
    private static toGasDegrees(date: UtcDate) : number {	
        var jd = date.daysSinceJ2000;
        var jt = jd / 36525.0;
        var jt2 = jt * jt;
        var jt3 = jt2 * jt;
...
//	            gms -= (0 | i) * 360.0;
    ...
    private static correctForRefraction(altitude : Angle | number) : Angle {
        var true_alt : any = altitude;
        
        if (true_alt instanceof Angle)
            true_alt = (<Angle>true_alt).degrees;
...
//				console.log('correctForRefraction: ' + (true_alt + R));
    ...
    private static uncorrectRefraction(altitude : Angle | number) : number {
        var true_alt : any = altitude;
...
//				console.log('uncorrectForRefraction: ' + (true_alt - R));
    ...
    private static toTopocentric(hc: Angle | number) : Angle {
        var alt_geoc :any = hc;
        
        if (alt_geoc instanceof Angle)
            alt_geoc = (<Angle>alt_geoc).degrees;
...
//			    console.log('toTopocentric: ' + alt_topoc);
    ...
    private static toGeocentric(hc: Angle | number) : Angle {
        var alt_topo : any = hc;
        
        if (alt_topo instanceof Angle)
            alt_topo = (<Angle>alt_topo).degrees;
...
//				console.log('toGeocentric: ' + alt_geoc);
    ...
    private static fromUnitVector(uv: Array<any>) : EquatorialCoordinates {
    ...
    private static precesionMatrixParams(date : UtcDate) : any {
        var jt = date.centuriesSinceJ2000;
        var jt2 = jt * jt;
        var jt3 = jt2 * jt;
...
//					
    ...
    private static determinant3x3(m : Array<Array<number>>) : number {
    ...
    private static inverse3x3(m: Array<Array<number>>) : Array<any> {
    ...
    private static inversePrecesionMatrixAt(date : UtcDate) : Array<any> {
    ...
    private static precesionMatrixAt(date : UtcDate) : Array<any> {
        var params = MeeusEngine.precesionMatrixParams(date);
    ...
    private static matrixTimesMatrix(P0 : Array<any>, P1 : Array<any>) : Array<any> {
        return [	
            [ 
    ...
    private static matrixTimesVector(P : Array<any>, p0 : Array<any>) : Array<any> {
        return [
    ...
    private static fromJ2000(ec: EquatorialCoordinates, date : UtcDate) : EquatorialCoordinates {
    ...
    private static toJ2000(ec: EquatorialCoordinates, date: UtcDate) : EquatorialCoordinates {
    ...
    private static toEpoch(ec : EquatorialCoordinates, date : UtcDate) : EquatorialCoordinates {
        var jt = date.centuriesSinceJ2000;	
        var jt2 = jt * jt;
        var jt3 = jt * jt2;
        ...
        if (raJNow < 0.0) raJNow += MeeusEngine.PI2;
        else if (raJNow > MeeusEngine.PI2) raJNow -= MeeusEngine.PI2;
    ...
    private static calc_SunriseOrSunset_UTC(isSunrise : boolean, jd : UtcDate, location: GeographicCoordinates, twilight: Twilight) : number {
    ...
    private static prepareJd(date: UtcDate | Date) : UtcDate {
        var jd : UtcDate;
        
        if (date instanceof Date) {
            jd = UtcDate.fromDate(<Date>date);
        } else if (date instanceof UtcDate) {
            jd = <UtcDate>date;
        } else {
            throw 'date';
    ...
    private static calc_SunriseOrSunset(isSunrise : boolean, jd : UtcDate, location : GeographicCoordinates, twilight : Twilight) : UtcDate {
        var date = jd.toDate();			
        ...
        if (typeof(newTimeUTC) == 'number') {
            ...
            {   
                var jdy: number;//previous sunrise/next sunset
                if (isSunrise) { // find previous sunrise
                ...
                if (isSunrise == true) { // find previous sunrise
                    jdy = MeeusEngine.calc_JDofNextPrevRiseSet(1, isSunrise, JD.days, latitude, longitude, timezone, dst)
                } else { // find next sunset
                    jdy =MeeusEngine. calc_JDofNextPrevRiseSet(0, isSunrise, JD.days, latitude, longitude, timezone, dst)
                }
    ...
    private static isLeapYear(yr : number) : boolean  {
        return ((yr % 4 == 0 && yr % 100 != 0) || yr % 400 == 0);
    }
    
    private static calcDoyFromJD(jd : number) : number {
    ...
    private static calcSunRa(jt : number, eclipticObliquityCorrection? : number, sunApparentLong? : number) : number {
    ...
    private static calc_Sun_Declination(jt : number, eclipticObliquityCorrection? : number, sunApparentLong? : number) : number {
        var e = eclipticObliquityCorrection || MeeusEngine.calc_Ecliptic_ObliquityCorrection(jt);
        var lambda = sunApparentLong || MeeusEngine.calc_Sun_ApparentLong(jt);
    ...
    private static calc_EquationOfTime(jt : number, ecliptic_ObliquityCorrection? : number, sunMeanLongitude? : number, earthOrbitEcc? : number, sunMeanAnomaly? : number) : number {
    ...
    private static getHRadians(twilight: Twilight) : number
    {
        var h = 0.0;
    ...
    private static calcHourAngleSunriseOrSunset(isSunrise : boolean, lat : number, solarDec: number, twilight: Twilight) : number
    {
    ...
    /*
        ...
        */
    private static isDaylightSavingsTime(date : Date) : boolean {
    ...
    private static calcAzEl(T : number, localtime : number, latitude : number, longitude : number, zone : number) : number
    {
