...
d.parse=function(a,e,f){return d.matchToAngle(d.match(a,e,f))};d.match=function(a,e,f){null!=a&&(a=a.trim());for(var h=null,k=0;k<d.RegexPatterns.length;k++){var b=(new RegExp(d.RegexPatterns[k],"g")).exec(a);if(null!=b){void 0===f&&(a=e[k],f=void 0!==a?a:d.RegexPatternStyleMap[k]);h={match:b,style:f};break}}if(null==h)throw"s";return h};d.matchToAngle=function(d){var e=d.match[2];e&&(e=e.replace(",","."));var h=parseFloat(e);isNaN(h)&&(h=0);(e=d.match[4])&&(e=e.replace(",","."));var k=parseFloat(e);
isNaN(k)&&(k=0);(e=d.match[6])&&(e=e.replace(",","."));e=parseFloat(e);isNaN(e)&&(e=0);switch(d.style){case f.Hour:h=a.fromHours(h,k,e);break;case f.Degree:h=a.fromDegrees(h,k,e);break;case f.Radian:h=a.fromRadians(h);break;default:throw"style";}"-"===d.match[1]&&(h=h.negative());return h};d.RegexPatterns="d ([+-])?(\\d{1,3}):( )?(\\d{1,2}):( )?(\\d{1,2}([\\.,]\\d+)?);([+-])?(\\d{1,3}):( )?(\\d{1,2}):( )?(\\d{1,2}([\\.,]\\d+)?);d ([+-])?(\\d{1,3})() (\\d{1,2})() (\\d{1,2}([\\.,]\\d+)?);([+-])?(\\d{1,3})() (\\d{1,2})() (\\d{1,2}([\\.,]\\d+)?);([+-])?(\\d{1,3})h( )?(\\d{1,2})m( )?(\\d{1,2}([\\.,]\\d+)?)[s]?;([+-])?(\\d{1,3})d( )?(\\d{1,2})m( )?(\\d{1,2}([\\.,]\\d+)?)[s]?;([+-])?(\\d{1,3})[\u00b0*]( )?(\\d{1,2})'( )?(\\d{1,2}([\\.,]\\d+)?)[\"]?;([a-zA-Z])(\\d{2})()(\\d{2})()(\\d{2}([\\.,]\\d+)?);([+-])?(\\d{2})()(\\d{2})()(\\d{2}([\\.,]\\d+)?);([+-])?(\\d{2})()(\\d{2}[\\.,]\\d+);[d][ ]([+-])?(\\d{1,3}([\\.\\,]\\d+)?);([+-])?(\\d{1,3}([\\.,]\\d+)?)".split(";");
